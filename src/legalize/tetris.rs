...
pub fn legalize(lp: &LegalProblem) -> Vec<LegalPosition> {
    ...
    for block in &blocks {
        ...
        let high_row = (best_row + 3).min(params.grid_y.saturating_sub(block_rows));
        ...
        for row in low_row..=high_row {
            // cal left edge
            let left = left_edges[row];
            
            let displacement = (block.x - left).abs() + 
                             (block.y - (params.origin_y + row as f32 * params.step_y)).abs();
        ...
        for r in best_row..best_row + block_rows {
            if left_edges[r] > legal_x {
                panic!("Overlap detected at row {}", r);
            }
        }
